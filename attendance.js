...
function renderAll() {
  ...
  const workDays = present + absent; // holidays not counted in rate
  ...
  renderDonut(recs, present, absent, holiday);
  renderBar(recs);
  renderTable(recs);
}

function renderDonut(recs, present, absent, holiday) {
  ...
  charts.donut = new Chart(ctx, {
    type: 'doughnut',
    data: {
      labels: ['Present', 'Absent', 'Holiday', 'Pending'],
      datasets: [{
        data: [present, absent, holiday, pending],
        backgroundColor: ['rgba(16,185,129,.75)','rgba(239,68,68,.75)','rgba(124,58,237,.6)','rgba(100,116,139,.4)'],
        borderColor: ['#10B981','#EF4444','#7C3AED','#94A3B8'],
        borderWidth: 1,
      }]
    },
    options: {
...
function renderBar(recs) {
  ...
  if (!ctx) return;
  let labels = [], presentData = [], absentData = [], holidayData = [];
  ...
  if (currentPeriod === 'today') {
    ...
    labels = days; presentData = new Array(7).fill(0); absentData = new Array(7).fill(0); holidayData = new Array(7).fill(0);
    recs.forEach(r => {
    ...
    const weeks = { 'Wk 1':{p:0,a:0,h:0},'Wk 2':{p:0,a:0,h:0},'Wk 3':{p:0,a:0,h:0},'Wk 4':{p:0,a:0,h:0},'Wk 5':{p:0,a:0,h:0} };
    recs.forEach(r => {
    ...
    const active = Object.entries(weeks).filter(([,v])=>v.p+v.a+v.h>0);
    labels = active.map(([k])=>k); presentData = active.map(([,v])=>v.p); absentData = active.map(([,v])=>v.a); holidayData = active.map(([,v])=>v.h);
  } else {
    const byMonth = {};
    recs.forEach(r => {
      const mo = r.date.substring(0,7);
      if (!byMonth[mo]) byMonth[mo]={p:0,a:0,h:0};
    ...
    presentData = sorted.map(m=>byMonth[m].p); absentData = sorted.map(m=>byMonth[m].a); holidayData = sorted.map(m=>byMonth[m].h);
  }
